...
function additionalPaymentTypes() { //Adds additionally supported card types
	
	jQuery.payment.cards.unshift(
	{
...
function transactiumFormHandler(event) {
    ...
    if ((jQuery('#payment_method_transactium_woocommerce_addon').is(':checked') && 'new' === jQuery( 'input[name="wc-transactium_woocommerce_addon-payment-token"]:checked' ).val()) || ('1' === jQuery('#woocommerce_add_payment_method').val())) {
		...
		if (!hack) 
		{
			...
			function invalidateField(field, invalid) { //mark invalid fields, restore valid fields
				...
				if (invalid) (jQuery(field).parent()).removeClass("woocommerce-validated").addClass("woocommerce-invalid");
				else (jQuery(field).parent()).removeClass("woocommerce-invalid").addClass("woocommerce-validated");
            ...
            function validateForm() {
				...
				var cardCVCValid = (cardType == "cashlink") ? //cashlink cards do not have cvc number
									(jQuery('#transactium_woocommerce_addon-card-cvc').val() == "") : //make sure cvc number for cashlink is empty
									jQuery.payment.validateCardCVC(jQuery('#transactium_woocommerce_addon-card-cvc').val(), cardType); //validate other cards' cvc number
		...
		/* This was removed due to not having a card number available when paying with previous payment method => to check the card type
		var cardType = jQuery.payment.cardType(jQuery('#transactium_woocommerce_addon-card-number').val());
		var cardCVCValid = (cardType == "cashlink") ? //cashlink cards do not have cvc number
									(jQuery('#transactium_woocommerce_addon-card-cvc').val() == "") : //make sure cvc number for cashlink is empty
									jQuery.payment.validateCardCVC(jQuery('#transactium_woocommerce_addon-card-cvc').val(), cardType); //validate other cards' cvc number
