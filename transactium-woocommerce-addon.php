...
{	
    ...
    // so do nothing
    if (!class_exists('WC_Payment_Gateway_CC') || !is_woocommerce_active() || class_exists('transactium_woocommerce_addon'))
        return;
    ...
    {		
        ...
        function __construct()
        {
			...
			$this->ezpay_result_statuses = array("Approved", "Declined", "Error", "Blocked", "Voided", "Dubious", "Enrolled", "Result Code Invalid");
            ...
            add_action('woocommerce_credit_card_form_end', array($this,'init_checkout'));
			
			//handler is called after 3dsecure (HOST) completes from transactium - End3DSHostPostbackURL
			add_action( 'woocommerce_api_wc_gateway_transactium_woocommerce_addon', array( $this, 'return_handler' ) );
			
			add_action('woocommerce_scheduled_subscription_payment_transactium_woocommerce_addon', array ( $this, 'scheduled_subscription_payment' ), 10, 2);
					...
					<section class="woocommerce-card-details">
						...
						<table class="woocommerce-table woocommerce-table-card-details shop_table card_details">
							<tr>
								...
								<td><?php echo $billing_full_name; ?></td>
							</tr>
							<tr>
								<th scope="row"><?php echo __('Card type'); ?>:</th>
								<td><?php echo get_post_meta($order_id, 'card_type', true); ?></td>
							</tr>
							<tr>
								<th scope="row"><?php echo __('Card number'); ?>:</th>
								<td><?php echo get_post_meta($order_id, 'card_number_masked', true); ?></td>
							</tr>
						</table>
					</section>
			...
			add_filter( 'woocommerce_email_customer_details_fields', function ($fields, $sent_to_admin, $order) {
				...
				if (get_post_meta($order_id, 'gateway_id', true) == $this->id && $this->checkout_card_details == "yes") {
					...
					$billing_full_name = $this->WC_compat($order, 'billing_first_name') . " " . $this->WC_compat($order, 'billing_last_name');
					$fields["card_holder_name"] = array( "label" => "Card holder name", "value" => $billing_full_name );
					$fields["card_type"] = array( "label" => "Card type", "value" => get_post_meta($order_id, 'card_type', true) );
					$fields["card_number"] = array( "label" => "Card number", "value" => get_post_meta($order_id, 'card_number_masked', true) );
		...
		public function checks() {
			...
			if ( version_compare( phpversion(), '5.3', '<' ) ) {
				echo '<div class="error"><p>' . sprintf( __( 'Transactium EZPay Error: Transactium EZPay requires PHP 5.3 and above. You are using version %s.', 'transactium-wc-addon' ), phpversion() ) . '</p></div>';
			}
        ...
        function do_admin_checks()
        {
            if ($this->enabled == "yes") {
                if ($this->WC_3() === null) {
					echo "<div class=\"error\"><p>" . sprintf(__("This version of <strong>%s</strong> requires WooCommerce v2.4 or later. Please upgrade WooCommerce.", 'transactium-wc-addon'), $this->method_title) . "</p></div>";
				}
        ...
        public function init_checkout($caller)
        {
			...
			wp_enqueue_script( 'transactium-wc-addon', plugins_url('assets/js/transactium-wc-addon.js',__FILE__), array( 'jquery', 'wc-credit-card-form' ), WC_VERSION, true );
			...
			if($this->get_three_d_secure() === "Host") {
				wp_enqueue_script('3DSecure', $url . '/Scripts/Crypto.js');
			}
...
?>
		...
		<input type="hidden" data-ezpay-token-type="amount-decimal" />

		<?php if($this->get_three_d_secure() === "MPI" && is_checkout()) { ?>
			<input type="hidden" data-ezpay-token-type="mode" value="MPI" />
			
		<?php } else if($this->get_three_d_secure() === "Host" && is_checkout()) { ?>
			<input type="hidden" data-ezpay-token-type="mode" value="Host" />
		<?php } ?>
	...
	<?php
		...
		public function add_payment_method() {
			...
			$payload = array(
                ...
                "RequestType" => "1" // 1 = PRE-AUTH
            );
			
			
			// Send this payload to Transactium for processing
            $response = wp_remote_post($this->get_base_url()."/webservice/ezpay/PayWithToken", array_merge( array(
                'method' => 'POST',
			...
			//if success, save transaction id using save_token
			...
			switch ($result_code) {
				...
				default:
					...
					wc_add_notice("PreAuth Transaction ".$this->ezpay_result_statuses[$result_code], 'error');
					return;
			}
        ...
        public function process_payment($order_id)
        {
            ...
            $payload = array(
                ...
                "RequestType" => "0" // 0 = SALE
            );
			
			$path = '';
			
			if (isset($_POST['wc-transactium_woocommerce_addon-payment-token']) && $_POST['wc-transactium_woocommerce_addon-payment-token'] === "new") {
			
				$payload['Token']=$_POST['Token'];
				$path="PayWithToken";
			...
			if($this->get_three_d_secure() === "Host" && $result_code === 6) {
				return array(
					'result' => "success",
					'redirect' => add_query_arg('token', urlencode($_POST['Token']), $this->get_base_url() . '/CardToken/PayerAuthentication')
				);
			}
			...
			switch ($result_code) {
				case 0:
					...
					if (isset($_POST['wc-transactium_woocommerce_addon-new-payment-method']) && $_POST['wc-transactium_woocommerce_addon-new-payment-method'] === "true") {
						$token = new WC_Payment_Token_CC();
						$token->set_token($transaction_id);
						$token->set_gateway_id($this->id);
		...
		public function return_handler() {
			...
			$result_code_message = $_POST['Response_Result_Code'];
			$transaction_id = $_POST['Response_Transaction_Id'];
			$transaction_authcode = $_POST['Response_Transaction_AuthenticationCode'];
			
			$result_code = array_search(strtolower($result_code_message), array_map('strtolower', $this->ezpay_result_statuses));		

			$order_id = absint($_REQUEST['reference']);
			$new_payment_method = $_REQUEST['new_payment_method'];
			$customer_order     = wc_get_order( $order_id );
			
			if ($result_code === null || $result_code === false)
				$result_code = 7;
			...
			switch ($result_code) {
				case 0:
					...
					update_post_meta( $order_id, 'transaction_id', $transaction_id );
					update_post_meta( $order_id, 'card_number_masked', $_POST['Response_Card_Bin']."****".$_POST['Response_Card_LastFourDigits']);
					update_post_meta( $order_id, 'card_type', $_POST['Response_Card_ApplicationProfile'] );
					update_post_meta( $order_id, 'gateway_id', $this->id );
					...
					if ($new_payment_method === "true") {
						...
						$token->set_card_type($_POST['Response_Card_ApplicationProfile']);
						$token->set_last4($_POST['Response_Card_LastFourDigits']);
						$token->set_expiry_month(substr($_POST['Response_Card_ExpiryYYMM'],2,4));
						$token->set_expiry_year('20'.substr($_POST['Response_Card_ExpiryYYMM'],0,2));
						$token->set_user_id(get_current_user_id());
						$token_saved = $token->save();
		...
		public function process_refund( $order_id, $amount = null, $reason = '' ) {
			...
			if (is_wp_error($response))
				return new WP_Error( 'transactium-wc-addon_refund_error',__('We are currently experiencing problems trying to connect to this payment gateway. Sorry for the inconvenience.', 'transactium-wc-addon'));
			
			if (empty($response['body']))
				return new WP_Error( 'transactium-wc-addon_refund_error',__('Gateway Response was empty.', 'transactium-wc-addon'));
